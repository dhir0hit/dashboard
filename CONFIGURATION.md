...
End-to-end guide for configuring the Docker Dashboard — from spinning up the
...
| A Docker host (optional) | Only needed when `MOCK=false` |
| `hermes` CLI on the backend host (optional) | Needed for the Calendar page to show real jobs |

Mock mode (`MOCK=true`, the default) needs **none** of the Docker/Hermes
...
To switch to a real Docker host later, keep reading.
...
| `MOCK` | `true` | `true` → canned mock data, no Docker host needed. `false` → query the real Docker host. |
...
The backend can enumerate Docker containers on the host. It uses the Docker
socket mounted into the backend container.

**Direct Docker socket** (default): the compose file already mounts
`${DOCKER_SOCK:-/dev/null}:/var/run/docker.sock`. This discovers containers
on the host. Comment out that line in `docker-compose.yml` to disable.
...
Docker-discovered services for status overlay and health polling.
...
| Method | Path | Purpose |
|---|---|---|
| `GET` | `/api/services` | List discovered services (mock or from Docker). |
...
Variables override as usual (`MOCK=false DOCKER_SOCK=... uvicorn ...`).
...
| No services discovered in real mode | Confirm the Docker socket is mounted and accessible. Check `docker compose logs backend` for errors. |
