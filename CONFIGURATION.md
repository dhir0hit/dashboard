...
End-to-end guide for configuring the Dashboard — from spinning up the
stack to wiring it to a real Docker host and tuning every through-the-website
...
| A Docker host with containers to discover (optional) | Only needed when `MOCK=false` |
| `/var/run/docker.sock` accessible to the backend (optional) | Needed for real service discovery |
| `hermes` CLI on the backend host (optional) | Needed for the Calendar page to show real jobs |

Mock mode (`MOCK=true`, the default) needs **none** of the Docker-socket/Hermes
...
To switch to real Docker container discovery later, keep reading.
...
| `MOCK` | `true` | `true` → canned mock data, no Docker socket needed. `false` → query the local Docker socket. |
| `DOCKER_SOCK` | `/var/run/docker.sock` | Host Docker socket for discovering containers. Set to `/dev/null` to disable. |
...
The backend discovers containers by talking to the local Docker socket.
The compose file mounts `${DOCKER_SOCK:-/dev/null}:/var/run/docker.sock` into
the backend container. When `MOCK=false`, the backend queries Docker for all
running and stopped containers via `docker ps -a` and returns them as
discoverable services.

To disable socket discovery, set `DOCKER_SOCK=/dev/null` (or remove the
volume mount) and the backend will return an empty discovery list.
...
discovered services for status overlay and health polling.
...
- **Add a tile**: go to **Settings → Dashboard tiles → Add tile**. Provide:
  ...
  - **Container ID** (optional) — link the tile to a discovered service id
    (e.g. `docker-grafana`) so live status/health is overlaid.
...
| Method | Path | Purpose |
|---|---|---|
| `GET` | `/api/services` | List discovered services (mock or from Docker socket). |
...
curl -X POST http://localhost:8888/api/config/services \
  ...
  -d '{"name":"Sonarr","url":"http://sonarr.example.com:8989","icon":"sonarr","container_id":"docker-sonarr"}'
```
...
Variables override as usual (`MOCK=false uvicorn ...`).
...
| Backend container exits immediately | Set `MOCK=true` (the default). |
...
| No services discovered in real mode | Confirm `/var/run/docker.sock` is mounted into the backend container. Check `docker compose logs backend` for errors. |
...
- [ ] `/health` returns `{"ok": true, "mock": bool}`.
- [ ] In real mode, `/api/services` reflects your actual Docker containers.
