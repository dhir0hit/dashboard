...
The Dashboard is a two-container stack: a FastAPI backend that discovers
Docker containers via the local Docker socket and does all persistence,
and a React + Vite + Tailwind frontend served by nginx that reverse-proxies
API calls to the backend so the user talks to exactly one origin.
...
5. If MOCK=true                     → backend returns MOCK_SERVICES
   If MOCK=false                    → discover_docker_services() via Docker socket
...
See [`../CONFIGURATION.md`](../CONFIGURATION.md) for env vars, the Docker
socket discovery setup, deploying with real Docker containers, and the
through-the-website configuration story (tiles, theme, background,
bookmarks, search, calendar).
...
| Docker socket not mounted | `/api/services` returns an empty list; Home shows the empty CTA or just the user's configured tiles. Check `docker compose logs backend` for errors. |
